...
В репозитории находятся Go API, PostgreSQL, SQL-миграции Goose, OpenAPI-контракт и Docker Compose для общего запуска. Контракт — в [api.yaml](api.yaml).
...
| Вопрос | Решение |
|---|---|
| Когда формировать слоты? | При запросе конкретной даты в пределах 365 дней. Прошедшие даты возвращают пустой список без записи в БД; ограниченный горизонт не даёт раздувать таблицу произвольными датами. |
...
Миграции применяются до запуска HTTP-сервера под PostgreSQL advisory lock и с двухминутным timeout. Запросы к БД получают контекст HTTP-запроса; пул ограничивает число соединений. Сервер задаёт таймауты чтения и записи, а при SIGINT/SIGTERM даёт активным запросам до 10 секунд на завершение. Каждый HTTP-запрос получает request ID и структурированный access-log; паники преобразуются в общий `INTERNAL_ERROR`.
...
Ошибки возвращаются в виде `{"error":{"code":"SLOT_ALREADY_BOOKED","message":"slot is already booked"}}`, включая неизвестные маршруты и неподдерживаемые HTTP-методы. Неверные параметры дают `400`, отсутствие действительного токена — `401`, недоступная роль или чужая бронь — `403`, отсутствующий ресурс — `404`, конфликт расписания или активной брони — `409`, временное исчерпание bcrypt-лимита — `503`.
